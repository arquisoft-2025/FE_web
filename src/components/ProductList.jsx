...
function ProductList() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // ✅ CORREGIDO: Usar result.data.getDonations
      const data = result.data?.getDonations || [];
      
      // ✅ CORREGIDO: Transformar datos para que coincidan con el formato esperado
      const transformedData = data.map(donation => ({
        ...
        image_url: donation.imageUrl, // Transformar imageUrl a image_url si es necesario
        imageUrl: donation.imageUrl,   // Mantener ambos para compatibilidad
      ...
      console.log('Products fetched via GraphQL:', transformedData);
  ...
  // ✅ CORREGIDO: Mapear categorías para que coincidan con las de GraphQL
  const mapCategoryFilter = (category) => {
    const categoryMap = {
      'Todos': 'Todos',
      'Comida': 'Alimentos', // Mapear Comida -> Alimentos
      'Ropa': 'Ropa',
  ...
  const filteredProducts = products.filter(p => {
    const mappedCategory = mapCategoryFilter(selectedCategory);
    const matchesCategory = mappedCategory === 'Todos' || p.category === mappedCategory;
    ...
    // ✅ CORREGIDO: Solo mostrar productos disponibles
    const isAvailable = p.available === true;
    
    return matchesCategory && matchesCity && matchesCondition && isAvailable;
  });
